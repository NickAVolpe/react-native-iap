...
export const prepare = () => {
  console.warn(
    'The `prepare` method is deprecated. Use initConnection method instead.'
  )
  Platform.select({
    ios: async () => {
...
export const getSubscriptions = skus =>
  Platform.select({
    ...
    android: async () => {
      console.log('checking Native android')
      checkNativeAndroidAvailable()
      let isAmazonDevice = checkIsAmazonDevice()
      if (isAmazonDevice) {
        console.log('checked done, skus:', skus)
        return RNIapAmazonModule.getProductData(skus)
      } else {
        return RNIapAndroidModule.getItemsByType(
  ...
  Platform.select({
    ...
    android: async () => {
      ...
      if (isAmazonDevice) {
        ...
        let products = await RNIapAndroidModule.getPurchaseHistoryByType(
          ANDROID_ITEM_TYPE_IAP
        )
        let subscriptions = await RNIapAndroidModule.getPurchaseHistoryByType(
          ANDROID_ITEM_TYPE_SUBSCRIPTION
        )
  ...
  Platform.select({
    ...
    android: async () => {
      ...
      if (isAmazonDevice) {
        ...
        let products = await RNIapAndroidModule.getAvailableItemsByType(
          ANDROID_ITEM_TYPE_IAP
        )
        let subscriptions = await RNIapAndroidModule.getAvailableItemsByType(
          ANDROID_ITEM_TYPE_SUBSCRIPTION
        )
        return products.concat(subscriptions)
      }
...
export const buySubscription = (sku, oldSku, prorationMode) => {
  return Platform.select({
    ...
    android: async () => {
      ...
      let isAmazonDevice = checkIsAmazonDevice()
      if (isAmazonDevice) {
        console.log('Purchasinging')
        let response = RNIapAmazonModule.purchase(sku)
        console.log('Purchasinging response:', JSON.stringify(response))
        return response
      } else {
